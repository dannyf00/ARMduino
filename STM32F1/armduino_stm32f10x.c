...
#include "armduino_stm32f10x.h"						//we use aruidno GPIO port for stm32F
...
#if defined(USE_TIM4)
//global variables
static void (* _tim4_oc1isrptr)(void)=empty_handler;				//tim3_ptr pointing to empty_handler by default
static void (* _tim4_oc2isrptr)(void)=empty_handler;				//tim3_ptr pointing to empty_handler by default
static void (* _tim4_oc3isrptr)(void)=empty_handler;				//tim3_ptr pointing to empty_handler by default
static void (* _tim4_oc4isrptr)(void)=empty_handler;				//tim3_ptr pointing to empty_handler by default

static uint32_t _tim4_oc1=0;				//output compare registers
static uint32_t _tim4_oc2=0;
static uint32_t _tim4_oc3=0;
static uint32_t _tim4_oc4=0;

//isr for timer1 capture / compare
void TIM4_IRQHandler(void) {
	//oc1 portion
	if (TIM4->SR & TIM_SR_CC1IF) {		//output compare 1 flag is set
		TIM4->SR &=~TIM_SR_CC1IF;		//clear the flag
		TIM4->CCR1 += _tim4_oc1;			//update the output compare register
		_tim4_oc1isrptr();				//execute user handler
	}

	//oc2 portion
	if (TIM4->SR & TIM_SR_CC2IF) {		//output compare 2 flag is set
		TIM4->SR &=~TIM_SR_CC2IF;		//clear the flag
		TIM4->CCR2 += _tim4_oc2;			//update the output compare register
		_tim4_oc2isrptr();				//execute user handler
	}

	//oc3 portion
	if (TIM4->SR & TIM_SR_CC3IF) {		//output compare 2 flag is set
		TIM4->SR &=~TIM_SR_CC3IF;		//clear the flag
		TIM4->CCR3 += _tim4_oc3;			//update the output compare register
		_tim4_oc3isrptr();				//execute user handler
	}

	//oc4 portion
	if (TIM4->SR & TIM_SR_CC4IF) {		//output compare 2 flag is set
		TIM4->SR &=~TIM_SR_CC4IF;		//clear the flag
		TIM4->CCR4 += _tim4_oc4;			//update the output compare register
		_tim4_oc4isrptr();				//execute user handler
	}
}
#endif
...
#if defined(USE_TIM6)
//global variables
static void (* _tim6_isrptr)(void)=empty_handler;				//tim4_ptr pointing to empty_handler by default

//isr for timer6 capture / compare
void TIM6_DAC_IRQHandler(void) {
	TIM6->SR &=~TIM_SR_UIF;		//clear the flag
	_tim6_isrptr();				//execute user handler
}
#endif

#if defined(USE_TIM7)
//global variables
static void (* _tim7_isrptr)(void)=empty_handler;				//tim4_ptr pointing to empty_handler by default

//isr for timer7 capture / compare
void TIM7_IRQHandler(void) {
	TIM7->SR &=~TIM_SR_UIF;		//clear the flag
	_tim7_isrptr();				//execute user handler
}
#endif
...
//*********************************************************
//SO MAKE SURE THAT THE PINS YOU PICKED _actUALLY EXIST FOR YOUR CHIP/PACKAGE
//*********************************************************
//Pin  0..15 -> GPIOA
//Pin 16..31 -> GPIOB
//Pin 32..47 -> GPIOC
...
inline void pinMode(PIN_TypeDef pin, uint8_t mode) {
	...
	case INPUT_PULLUP: 	GPIO_DDR(GPIO_PinDef[pin].gpio, GPIO_PinDef[pin].mask, GPIOMODE_INPU); /*GIO_SET(GPIO_PinDef[pin].gpio, GPIO_PinDef[pin].mask); */break;			//input with pullup
	case INPUT_PULLDN:	GPIO_DDR(GPIO_PinDef[pin].gpio, GPIO_PinDef[pin].mask, GPIOMODE_INPD); /*GIO_CLR(GPIO_PinDef[pin].gpio, GPIO_PinDef[pin].mask); */break;			//input with pulldown
...
void attachInterrupt(PIN_TypeDef pin, void (*isrptr)(void), uint8_t mode) {
    ...
    switch (pos) {
    case 0: NVIC__setpriority(EXTI0_IRQn, 15); NVIC_EnableIRQ(EXTI0_IRQn); break;
    case 1: NVIC__setpriority(EXTI1_IRQn, 15); NVIC_EnableIRQ(EXTI1_IRQn); break;
    case 2: NVIC__setpriority(EXTI2_IRQn, 15); NVIC_EnableIRQ(EXTI2_IRQn); break;
    case 3: NVIC__setpriority(EXTI3_IRQn, 15); NVIC_EnableIRQ(EXTI3_IRQn); break;
    case 4: NVIC__setpriority(EXTI4_IRQn, 15); NVIC_EnableIRQ(EXTI4_IRQn); break;
    case 5:
    case 6:
    case 7:
    case 8:
    case 9: NVIC__setpriority(EXTI9_5_IRQn, 15); NVIC_EnableIRQ(EXTI9_5_IRQn); break;
    case 10:
    case 11:
    case 12:
    case 13:
    case 14:
    case 15: NVIC__setpriority(EXTI15_10_IRQn, 15); NVIC_EnableIRQ(EXTI15_10_IRQn); break;
    default: break;		//do nothing
    }

    //_activate the interrupt
    EXTI->PR |= (1<<pos);                           //clear the flag
    EXTI->IMR |= (1<<pos);                          //enable the exti
}

//de_activate an interrupt
void detachInterrupt(PIN_TypeDef pin) {
    ...
    switch (pos) {
    case 0: NVIC__setpriority(EXTI0_IRQn, 15); NVIC_DisableIRQ(EXTI0_IRQn); break;
    case 1: NVIC__setpriority(EXTI1_IRQn, 15); NVIC_DisableIRQ(EXTI1_IRQn); break;
    case 2: NVIC__setpriority(EXTI2_IRQn, 15); NVIC_DisableIRQ(EXTI2_IRQn); break;
    case 3: NVIC__setpriority(EXTI3_IRQn, 15); NVIC_DisableIRQ(EXTI3_IRQn); break;
    case 4: NVIC__setpriority(EXTI4_IRQn, 15); NVIC_DisableIRQ(EXTI4_IRQn); break;
    case 5:
    case 6:
    case 7:
    case 8:
    case 9: NVIC__setpriority(EXTI9_5_IRQn, 15); NVIC_DisableIRQ(EXTI9_5_IRQn); break;
    case 10:
    case 11:
    case 12:
    case 13:
    case 14:
    case 15: NVIC__setpriority(EXTI15_10_IRQn, 15); NVIC_DisableIRQ(EXTI15_10_IRQn); break;
    default: break;		//do nothing
    }
...
void tim4_init(uint32_t ps) {
	...
	//disable the interrupt by clearing the enable bits
	TIM4->DIER &=~(TIM_DIER_CC1IE | TIM_DIER_CC2IE | TIM_DIER_CC3IE | TIM_DIER_CC4IE);
...
void tim4_act1(void (*isr_ptr)(void)) {
	...
	_tim4_oc1isrptr = isr_ptr;			//install user handler
...
void tim4_act2(void (*isr_ptr)(void)) {
	...
	_tim4_oc2isrptr = isr_ptr;			//install user handler
...
void tim4_act3(void (*isr_ptr)(void)) {
	...
	_tim4_oc3isrptr = isr_ptr;			//install user handler
...
void tim4_act4(void (*isr_ptr)(void)) {
	...
	_tim4_oc4isrptr = isr_ptr;			//install user handler
...
void tim6_init(uint32_t ps) {
	...
	//source from internal clock -> disable slave mode
	TIM6->SMCR &=~TIM_SMCR_SMS;			//clear sms->use internal clock

	//stop the timer to configure it
	TIM6->CR1 &=~TIM_CR1_CEN;			//clear cen. 0=disable the timer, 1=enable the timer
	TIM6->CR1 &=~TIM_CR1_CKD;			//clear CKD0..1. 0b00->1x clock; 0b01->2:1 clock, 0b10->4:1 clk; 0b11->reserved
	TIM6->CR1 &=~TIM_CR1_DIR;			//clear DIR bit. 0=upcounter, 1=downcounter
	TIM6->CR1 &=~TIM_CR1_OPM;			//clear opm bit. 0=periodic timer, 1=one-shot timer
	//or to simply zero the register
	//TIM6->CR1 = 0;						//much easier

	//clear the status register bits for capture / compare flags
	TIM6->SR &=~(TIM_SR_CC1IF | TIM_SR_CC2IF | TIM_SR_CC3IF | TIM_SR_CC4IF | TIM_SR_UIF);
	//disable the interrupt by clearing the enable bits
	TIM6->DIER &=~(TIM_DIER_CC1IE | TIM_DIER_CC2IE | TIM_DIER_CC3IE | TIM_DIER_CC4IE | TIM_DIER_UIE);

	//set the prescaler
	TIM6->PSC = ps - 1;					//set the prescaler
	TIM6->RCR = 0;						//repetition counter = 0 (=no repetition)
	TIM6->ARR = -1;						//auto reload register / period = 0; - need to change for downcounters
...
void tim6_setpr(uint32_t pr) {
	...
	//clear the flag
	//TIM6->SR &=~TIM_SR_CC1IF;			//clear the interrupt flag
	//TIM6->DIER &=~TIM_DIER_CC1IE;		//disable the isr
}
...
void tim6_act(void (*isr_ptr)(void)) {
	...
	_tim6_isrptr = isr_ptr;			//install user handler
...
#if defined(USE_TIM7)
//initialize tim7 to use compare channels as timers
//16-bit prescaler. 32-bit used for compatability
void tim7_init(uint32_t ps) {
	...
	//source from internal clock -> disable slave mode
	TIM7->SMCR &=~TIM_SMCR_SMS;			//clear sms->use internal clock

	//stop the timer to configure it
	TIM7->CR1 &=~TIM_CR1_CEN;			//clear cen. 0=disable the timer, 1=enable the timer
	TIM7->CR1 &=~TIM_CR1_CKD;			//clear CKD0..1. 0b00->1x clock; 0b01->2:1 clock, 0b10->4:1 clk; 0b11->reserved
	TIM7->CR1 &=~TIM_CR1_DIR;			//clear DIR bit. 0=upcounter, 1=downcounter
	TIM7->CR1 &=~TIM_CR1_OPM;			//clear opm bit. 0=periodic timer, 1=one-shot timer
	//or to simply zero the register
	//TIM7->CR1 = 0;						//much easier

	//clear the status register bits for capture / compare flags
	TIM7->SR &=~(TIM_SR_CC1IF | TIM_SR_CC2IF | TIM_SR_CC3IF | TIM_SR_CC4IF | TIM_SR_UIF);
	//disable the interrupt by clearing the enable bits
	TIM7->DIER &=~(TIM_DIER_CC1IE | TIM_DIER_CC2IE | TIM_DIER_CC3IE | TIM_DIER_CC4IE | TIM_DIER_UIE);

	//set the prescaler
	TIM7->PSC = ps - 1;					//set the prescaler
	TIM7->RCR = 0;						//repetition counter = 0 (=no repetition)
	TIM7->ARR = -1;						//auto reload register / period = 0; - need to change for downcounters
	TIM7->CNT = 0;						//reset the counter
...
void tim7_setpr(uint32_t pr) {
	...
	//clear the flag
	//TIM7->SR &=~TIM_SR_CC1IF;			//clear the interrupt flag
	//TIM7->DIER &=~TIM_DIER_CC1IE;		//disable the isr
}
...
void tim7_act(void (*isr_ptr)(void)) {
	...
	_tim7_isrptr = isr_ptr;			//install user handler
...
void chip_init(void) {
	...
	ADC1->CR1 = (0<<23) |					//0->disable analog watchdog on regular channels
				...
				0x00;
	ADC1->CR2 = (1<<23) |					//0->temperature sensor / internal vref disabled
