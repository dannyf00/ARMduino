...
uint32_t millis(void) {
	...
	return (m - f) / clockCyclesPerMillisecond();					//SysTick is a 24-bit downcounter
}
...
void systick_delayus(uint32_t us) {
	...
	while (ticks() - start_time < us) continue;	//wait until desired time runs out
}
//end Time
...
void mcu_init(void){
	...
	SysTick->LOAD  = 	(0xfffffful/*ticks*/ & SysTick_LOAD_RELOAD_Msk) - 0;      /* set reload register */
